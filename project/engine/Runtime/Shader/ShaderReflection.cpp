...
void RootSignatureBuilder::BuildFromReflection(const std::vector<ShaderReflection>& reflections, RootSignature& rootSig) {
	...
	std::unordered_map<std::string, MergedResource> merged;
	bool hasVS = false;
	...
	for (auto& refl : reflections) {
		...
		for (auto& r : refl.resources_) {
			...
			if (merged.contains(r.name)) {
				// ToDo : 既に存在 → bindPoint や space が違う場合は警告しても良い
				continue;
			}
