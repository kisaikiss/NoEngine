	...
	if (timer_ > sGenerateTime) {
        ...
        for (uint32_t i = 0; i < generateCount; i++) {
            ...
            if (isGreen_) {
                ...
                No::ModelLoader::LoadModel(resource_.greenName, resource_.greenModelPath, model, animationComp);
                m->materials = No::ModelLoader::GetMaterial(resource_.greenName);

            } else {
                registry.AddComponent<BatTag>(entity);
                No::ModelLoader::LoadModel(resource_.modelName, resource_.modelPath, model, animationComp);
                m->materials = No::ModelLoader::GetMaterial(resource_.modelName);
            }
