...
### 検討テーマ
- [x] Q1: 当たり判定デバッグ表示（敵/自機/フィールド）を一括管理する方式の可否を評価する
- [x] Q2: 衝突判定を `CollisionSystem` に集約し、`OnCollision` 的な通知で各System責務を分離する方式を評価する

### 検討ステップ
- [x] 現行の衝突関連責務を棚卸しする（判定/命中処理/演出/デバッグ表示）
- [x] 一括デバッグ表示の導入コストと既存影響を見積もる
- [x] `OnCollision` イベント方式の最小導入案を作る
- [x] 過分離による煩雑化を避ける責務境界案を作る
- [x] 採用/不採用を判断し、採用時は次フェーズへ昇格する

### Phase12 結論（精査結果）
- [x] Q1（一括デバッグ表示）は **採用**
  - 根拠: 現在は `EnemyVisualSystem` と `FieldEditorSystem` にトグル・描画責務が分散しており、確認導線が分かれている
  - 根拠: 描画基盤は既存 `Primitive` + ImGui で足りるため、新規基盤追加なしで統合可能
- [x] Q2（`CollisionSystem` + `OnCollision`）は **段階導入で採用**
  - 根拠: 現在の `CollisionSystem` は「判定とフラグ更新」止まりで、命中解決は `EnemyBulletHitSystem` / `EnemyContactDamageSystem` / `PlayerAttackResolveSystem` に分散
  - 根拠: 既存 `EventBus` があり、`DamageRequestComponent` のような段階移行パターンも既に運用実績がある
  - 注意: `PlayerAttackResolveSystem` の遮蔽・可視判定は仕様密度が高いため、一括移管せず段階化する

### 責務境界（採用案）
- [x] `CollisionSystem`: コライダー同期・投影・接触判定・衝突イベント発行
- [x] Resolver系System: イベント購読後にダメージ/消滅/報酬などゲームルールを適用
- [x] Debug表示System: 衝突情報の可視化（敵/自機/フィールド）を一元管理

### 完了条件
- [x] 一括デバッグ表示を「採用」で判断済み
- [x] 衝突処理の集約方針を「段階導入案」まで確定
- [x] 次の実装フェーズへ進める判断材料を明文化
...
### 実装ステップ
- [ ] `CollisionDebugConfigComponent`（仮称）を追加し、衝突デバッグ表示トグルを一元化する
- [ ] `CollisionDebugRenderSystem`（仮称）を追加し、敵/自機/フィールドの判定表示を集約する
- [ ] `EnemyVisualSystem` から衝突デバッグ描画責務を段階的に分離する（敵個別UIは維持）
- [ ] `FieldEditorSystem` から衝突デバッグ描画責務を分離し、トグル参照先を統一する
- [ ] `CollisionEvent`（仮称）を定義し、`CollisionSystem` で3D/3D・投影3D/2Dのイベントを発行する
- [ ] `EnemyBulletHitSystem` をイベント購読方式へ切替える（直参照フラグとの二重運用期間を設ける）
- [ ] `EnemyContactDamageSystem` をイベント購読方式へ切替える（連続接触の再発防止仕様を維持）
- [ ] `PlayerAttackResolveSystem` は遮蔽・可視判定を維持したまま、接触入力のみイベント連携へ段階移行する
- [ ] 旧来の `isColliding` 直参照コードを整理し、最終的な参照元を統一する
- [ ] 回帰確認（被弾・敵弾消滅・敵接触・遮蔽・デバッグ表示）を実施し、Phase13完了判定を行う

### 実装ルール（Phase13）
- [ ] 一度に全面移行しない（`EnemyBulletHit` → `EnemyContact` → `PlayerAttackResolve` の順で段階適用）
- [ ] 既存ゲーム仕様（カメラゲート、遮蔽、自然消滅時の挙動差）は変えない
- [ ] 不整合検出のため、移行期間はイベント経由と従来経由の差分ログを確認できるようにする

### 完了条件
- [ ] 衝突デバッグ表示の操作導線が1か所に統一される
- [ ] ダメージ/消滅判定の起点が `CollisionEvent` 経由へ移行される
- [ ] 既存仕様（遮蔽・ゲート・報酬条件）を維持したまま動作する
- [ ] C++14でビルド成功し、主要導線の回帰確認が完了する

---

## 14. 当たり判定再設計（被弾判定一本化フェーズ）

### 目的
- [ ] 自機 vs 敵 / 自機 vs 敵弾の被弾条件を「スプライト重なり + カメラゲート」に一本化する
- [ ] イベント経由と従来経由の多経路判定を廃止し、再現性を安定化する
- [ ] `isColliding` の広域参照による意図しない弾消滅を解消する
- [ ] マスク定義の命名を `CollisionMask` 側へ寄せ、追跡しやすくする

### 実装ステップ
- [ ] `CollisionMask.h` に CommentBout 用 `LayerPreset` / `MaskPreset` を追加し、命名を統一する
- [ ] `CollisionAlgorithms` に「投影重なり + カメラゲート」共通判定関数を追加する
- [ ] `EnemyBulletHitSystem` の被弾判定を共通関数へ統一し、イベント経路依存を除去する
- [ ] `EnemyBulletHitSystem` の弾消滅理由を明示化する（自機命中 / フィールド命中 / 後方消滅）
- [ ] `EnemyContactDamageSystem` の自機接触判定を共通関数へ統一する
- [ ] `PlayerAttackResolveSystem` の多経路判定を整理し、不要なイベント依存を除去する
- [ ] 必要箇所に日本語コメントを追記して、判定意図を明確化する

### 完了条件
- [ ] 自機の手前で弾が不自然に消える挙動が解消される
- [ ] 「重なっているのにダメージが入らない」再現が大幅に減る
- [ ] 自機被弾判定の条件が2Systemで同一実装になる
- [ ] C++14でビルド成功する
