...
void FieldObjectEditor::Update(No::Registry& registry)
{
	...
	if (loadedRegistry_ != &registry || loadedStageName_ != stageName) {
		LoadTypeDefinitions(typeDefinitions_);
		LoadFieldPlacements(registry, *resources, typeDefinitions_, stageName, idCounter_);
		loadedRegistry_ = &registry;
		loadedStageName_ = stageName;
		selectedEntity_ = No::nullEntity;
	}

	UpdateLoadedFieldObjects(registry, *resources, typeDefinitions_);
}

void FieldObjectEditor::DrawImGui(No::Registry& registry)
{
#ifdef USE_IMGUI
	GameResourceComponent* resources = nullptr;
	for (auto entity : registry.View<CBGameResourceTag, GameResourceComponent>()) {
	...
	const std::string& stageName = loadedStageName_.empty() ? "Stage_01" : loadedStageName_;
	...
	for (const auto& pair : typeDefinitions_) {
		typeKeys.push_back(pair.first);
	}
	std::sort(typeKeys.begin(), typeKeys.end());
	if (addTypeIndex_ >= static_cast<int>(typeKeys.size())) {
		addTypeIndex_ = 0;
	}
	...
	if (!typeKeys.empty()) {
		...
		ImGui::Combo("追加種別", &addTypeIndex_, labels.data(), static_cast<int>(labels.size()));
		if (ImGui::Button("フィールドオブジェクト追加")) {
			const std::string& key = typeKeys[addTypeIndex_];
			auto it = typeDefinitions_.find(key);
			if (it != typeDefinitions_.end()) {
				const std::string objectId = "field_obj_" + std::to_string(idCounter_++);
				const std::string displayName = key + "_" + std::to_string(idCounter_);
				selectedEntity_ = CreateFieldObjectEntity(
					registry,
					*resources,
					it->second,
					objectId,
	...
	for (auto entity : entities) {
		...
		if (ImGui::Selectable(label, selectedEntity_ == entity)) {
			selectedEntity_ = entity;
		}
	}

	if (selectedEntity_ != No::nullEntity && registry.Has<FieldPlacementComponent>(selectedEntity_) && registry.Has<No::TransformComponent>(selectedEntity_)) {
		auto* placement = registry.GetComponent<FieldPlacementComponent>(selectedEntity_);
		auto* transform = registry.GetComponent<No::TransformComponent>(selectedEntity_);
		if (placement && transform) {
			ImGui::Separator();
			...
			if (ImGui::Button("選択オブジェクト削除")) {
				registry.DestroyEntity(selectedEntity_);
				selectedEntity_ = No::nullEntity;
			}
		}
	}
...
void FieldObjectEditor::DrawTypeDefaultsImGui(No::Registry& /*registry*/)
{
#ifdef USE_IMGUI
	...
	if (ImGui::Button("種別デフォルト保存")) {
		SaveTypeDefinitions(typeDefinitions_);
	}

	ImGui::Separator();
	for (auto& pair : typeDefinitions_) {
	...
	ImGui::Separator();
	ImGui::TextDisabled("遮蔽仕様(第1段階): プレイヤー攻撃のみ対象");
#endif
}

void FieldObjectEditor::ForceReload()
{
	loadedRegistry_ = nullptr;
	loadedStageName_ = "";
}
