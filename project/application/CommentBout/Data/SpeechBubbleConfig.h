...
struct SpeechBubbleSizeConfig {
	...
	int   attackPower    = 1;
	float duration       = 0.9f;
	float appearDuration = 0.35f;  // 出現バウンスアニメーション時間（秒）
	float stopDuration   = 0.60f;  // 出現後の停止時間（秒）
};
...
struct SpeechBubbleConfig {
	...
	float initialScalePercent = 0.3f;  // 出現開始時スケール比率（0〜1）

	SpeechBubbleSizeConfig sizeLarge  = { { 120.f, 120.f }, 3, 0.9f, 0.35f, 0.60f };
	SpeechBubbleSizeConfig sizeMedium = { {  80.f,  80.f }, 2, 0.9f, 0.35f, 0.60f };
	SpeechBubbleSizeConfig sizeSmall  = { {  48.f,  48.f }, 1, 0.9f, 0.35f, 0.60f };
};
...
inline void to_json(nlohmann::json& j, const SpeechBubbleSizeConfig& s) {
	...
	j["appearDuration"]  = s.appearDuration;
	j["stopDuration"]    = s.stopDuration;
}
inline void from_json(const nlohmann::json& j, SpeechBubbleSizeConfig& s) {
	...
	if (j.contains("appearDuration")) s.appearDuration = j["appearDuration"].get<float>();
	if (j.contains("stopDuration"))   s.stopDuration   = j["stopDuration"].get<float>();
}
...
inline void to_json(nlohmann::json& j, const SpeechBubbleConfig& c) {
	...
	j["initialScalePercent"] = c.initialScalePercent;
...
inline void from_json(const nlohmann::json& j, SpeechBubbleConfig& c) {
	...
	if (j.contains("initialScalePercent")) c.initialScalePercent = j["initialScalePercent"].get<float>();
