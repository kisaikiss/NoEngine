...
VertexShaderOutput main(VertexShaderInput input)
{
    ...
    normal = skinnedNor;
#endif
    float distanceFactor = tan(gCameraMatrix.fov) * 0.0015; // 押し出し係数

    float4 worldPos = mul(position, gWorldMatrix.world);
    float3 worldNormal = normalize(mul(normal, (float3x3) gWorldMatrix.world));

    
    // カメラとの距離を計算
    float distance = length(gCameraMatrix.worldPosition - worldPos.xyz);

    // 距離に応じた押し出し量
    float dynamicOffset = distance * distanceFactor;

    // 法線方向へオフセット
    float3 normalOffset = normalize(worldNormal) * dynamicOffset * 2.0;

    float3 expandedPosition = position.xyz + normalOffset;
    
    float4 finalPos = mul(float4(expandedPosition, 1.f), gWorldMatrix.world);
    
    output.position = mul(finalPos, gCameraMatrix.viewProjection);
